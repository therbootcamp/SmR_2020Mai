...
2. Das Zeil beider Wettbewerbe ist es den kleinstmöglichen <i>p</i>-Wert durch das fortwährende Massieren der experimentellen Daten von Bem's zu extrahieren. Finde dafür bis zu drei Analysen, die zeigen, dass a) Menschen *psi*-Fähigkeiten besitzen, d.h., dass die `hit_rate` höher als Chance ist und b) dass es andere Variablen gibt die *psi*-Fähigkeiten prädizieren, was ebenso ein Beleg für die Existenz solcher Fähigkeiten wäre. Es gibt keine Regeln, ausser dass `hit_rate` als Kriterium zu wählen ist. Du kannst jeden möglichen Test verwenden. Dir ist es auch erlaubt nur Komponenten der Daten mit `data %>% filter(condition)` auszuwählen.

3. Für jede der beiden Wettbewerbe - jeweils ein Wettbewerb pro Experiment - darfst du drei Ergebnisse einreichen. Ihr könnt alleine arbeiten oder euch zusammenschliessen. Mehr Informationen über den Datensatz findet ihr unter dem  *Datasets* tab. Die Person oder Gruppe mit dem niedrigsten <i>p</i>-Wert gewinnt &#x1F36B;&#x1F36B;&#x1F36B;.  

4. Reiche die Ergebnisse ein über:


<table align="center">
  <tr>
    <td>Wettbewerb 1</td>
    <td><a href="https://docs.google.com/forms/d/e/1FAIpQLScw5MmBX9wnmv3YO7SzdqdWMWSSwdCjWu0EUM2kurLRJLpEOQ/viewform?usp=sf_link"><font style="font-size:18px;font-weight:900">Einreichen</font></a></td>
  </tr>
    <td>Wettbewerb 2</td>
    <td><a href="https://docs.google.com/forms/d/e/1FAIpQLSeJDsFtOSCFx7KmjzG4qq2bBMArPMWdaFKmUOqFfthspDhshQ/viewform?usp=sf_link"><font style="font-size:18px;font-weight:900">Einreichen</font></a></td>
  </tr>
</table>
...
5. Die letzte Analyse, das bestimmen der Power für einen gegebenen Effekt und Stichprobengrösse nennt man eine Post-hoc Poweranalyse, welche nicht mit einer eigentlichen Poweranalyse verwechselt werden darf. In Bem's Fall war die Power `.547`, das heisst das Bem nur ungefähr eine 50/50 Chance hatte einen Effekt von der gefundenen Stärke zu beobachten. Überprüfe nun was gewesen wäre, wenn der Effekt klein gewesen wäre, d.h., `d = .1`. Wiederhole die Powerberechnung für Bem's Stichprobengrösse und berechne die Stichprobengrlsse die nötig ist für eine $power = 1-\beta = .95$.      

```{r}
# Bems post-hoc power
power <- pwr.t.test(n = length(hit_rate_erotic), 
                    d = .1, 
                    sig.level = .05, 
                    alternative = "greater")
power
...
3. Das Konfidenezinterval für den Unterschied liegt zwischen zwischen `-6.84` und `.23`. Das Konfidenzintervalle enthält also die NUll. Das heisst, dass der obgleich der Mittelwert für die Erotic-Bedingung signifikant von `50%` verschieden ist die beiden Bedingungen sich nicht signifikant unterscheiden. Beginne nun die das Konfidenzinterval für den Unterschied der Gruppen nachzubauen. Extrahiere dafür zunächst dene Unterschied, den entsprechenden Standardfehler, und die Freiheitsgrade. 
...
4. Jetzt benötigst du nur noch die entsprechenden t-Wert um das Konfidenzintervall auzuspanne. Da das Konfidenzinterval in beide Richtungen zu gleichen Teilen aufgespannt werden soll such wir nicht den t-Wert für $1-\alpha$ sondern für $1-\frac{\alpha}{2}$ und $\frac{\alpha}{2}$, d.h., wir suchen die kritischen t-Werte für einen zweiseitigen Test mit $\alpha = .05$. Verwende das Template
...
5. Schaue dir zunächst die beiden t-Werte an. Überrascht, dass es die selben Werte sind nur einmal positiv und einmal negativ? Das ergibt sich daraus, dass die t-Verteilung symmetrisch um Null liegt. Um nun damit das Konfidenzinterval zu berechnen verwende das Template unten. Es geht darum `t*se` nach oben und `t*se` nach unten vom Unterschied `delta` zu gehen. Denkt an die unterschiedlichen Vorzeichen der beiden t-Werte.
...
```{r}
# confidence limits
delta_lower <- t_.25 + XX * XX
delta_upper <- t_97.5 + XX * XX
```

8. Jetzt könnt ihr eurer Konfidenzinterval mit dem aus dem t-Wert vergleichen. Ist es identisch?
...
2. Printe das Objekt und beetrachte den Output. Er sieht sehr anders aus als der Output der normalen Regression. Trotzddem solltest du einen Part wiedererkennen. Im Teil nach dem ersten mal `-----` stehen die Gewichte und deren Standardabweichung, welche du analog zum Standardfehler interpretieren kannst. Darüber stehen grundlegenden Informationen zum Modell, die du üblciherweise ignoriere kannst. Darunter stehen zu `Auxiliary parameter(s)` stehen Parameter, die das Modell zusätzlich geschätzt hat. Der eine Parameter `sigma` ist die Standardabweichung um die Regressionsgerade, welche im Bayesianischen Modell auch eine Standardabweichung besitzt. 

3. Statistische Entscheidungen werden im Bayesianischen Ansatz auf unterschiedliche Weisen getroffen (je nachdem welcher Schule man angehört). Am verbreitesten ist jedoch das *credible interval*, welches analog zum Konfidenzintervall zu interprieren ist. Es ist dasjenige Interval in dem der wahre Populationsparameter mit 95%iger Wahrscheinlichkeit liegt, eine Aussage, die für das normale Konfidenzinterval nicht getroffene werden kann. Berechne die *credible intervals* mit der `posterior_interval()` Funktion für `Trefferrate_bm`.    

```{r, message=F, warning=F}
# Bayesian posterior credible interval
posterior_interval(Trefferrate_bm)
```
...
|:----|:-----|:------|
|[psi_exp1.csv](https://therbootcamp.github.io/SmR_2020Apr/1_Data/psi_exp1.csv) | 200 | 7 |
|[psi_exp2.csv](https://therbootcamp.github.io/SmR_2029Apr/1_Data/psi_exp2.csv)  | 150 | 5 |

Die Daten stammen von einer tatsächlichen (para-) psychologischen Studie, die die menschliche Fähigkeit in die Zukunft zu sehen überprüft hat (["Feel the future"](https://therbootcamp.github.io/SwR_2019Apr/_sessions/NewStats/literature/Bem_Psi_2011.pdf)). Die mittlerweile berüchtigte Studie wurde in einem etablierten Journal der Sozialpsychologie publiziert, was damit nicht unerheblich zum einer Bewegung der methodischen Revision in der Psychologie geführt hat. Die Dateien enthalten die Daten der ersten beiden Experimente. Das erste Experiment untersuchte ob Menschen besser als eine Zufallsrate von 50% vorhersagen können, hinter welchem von zwei Vorhängen ein Bild versteckt ist, insbesondere in dem Fall in dem sich ein erotischen Bild dahinter versteckt. Experiment zwei ist ein Replikationsversuch der erste Studie. Beide Studien haben auch das Konstruk *Stimulus seeking* mituntersucht, was ähnlich zu Extraversion zu interpetieren ist, um zu untersuchen, ob Leute mit hohem *Stimulus seeking* vielleicht höhere *psi* Fähigkeiten aufweisen. 
