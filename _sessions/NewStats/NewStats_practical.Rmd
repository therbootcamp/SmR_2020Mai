...
4. Mittels des Plots könnt ihr nun auch evaluieren, wie gross die Power von Bem's ursprünglicher Studie war. In Bem's `'erotic'` condition waren $r length(Trefferrate_erotic)$ Beobachtungen. Finde den entsprechenden Punkt auf der x-Achse und gehe dann nach oben um die dazugehörige Power zu finden. Alternativ kannst du  

Using the plot, you can also already get an idea, how large the power, i.e., the probability of detecting an effect given that it is truly there to detect, was for Bem's study. You can also again use the `pwr.t.test()` function. To do this, set `n = length(hit_rate_erotic)` and `power = NULL`. Call the result `pa_bem`.
...
4. Now let's run the same analysis using the `BayesFactor` package using the `regressionBF` function.  
