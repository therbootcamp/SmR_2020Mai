...
<tr>
  ...
  <td bgcolor="white">
    Drug and placebo do <b><i>not</i></b> have the same effect
  </td>  
</tr>
</table>
...
<b>Question</b><br>
<i>Does "Drug X" reduce flu recovery time versus a placebo?</i>

<b>Hypotheses</b><br>


$$\large H_{0}: \mu_{Drug\;X} = \mu_{Placebo}$$
$$\large H_{1}: \mu_{Drug\;X} \neq \mu_{Placebo}$$
...
id <- 1:50
drug <- sample(c("2.Drug X", "1.Placebo"), size = 50, replace = TRUE)
effect <- rnorm(50)
effect[drug == "2.Drug X"] <- 2.3 + rnorm(sum(drug == "2.Drug X"), mean = 0, sd = 1.5) - .41
effect[drug == "1.Placebo"] <- 3.3 + rnorm(sum(drug == "1.Placebo"), mean = 0, sd = 1.5) - .12
...
drug_placebo_gg <- ggplot(dat, aes(factor(drug), effect)) +
  ...
  labs(y = "Days",
       x = "Condition",
       title = "Flu recovery time in days",
       caption = "Note: Fictional data") +
  scale_y_continuous(breaks = seq(0, 10, 1), limits = c(0, 7)) +
  geom_label(data = avg, aes(x = drug, y = days_mean, label = round(days_mean, 2)))
```

```{r, echo = FALSE, fig.width = 3, fig.height = 2.5, out.width="70%", warning=F, dpi=300, fig.align = 'center'}
drug_placebo_gg
```
...
$$\Large y = \beta_{0} + \sum_{i=1}^{n}\beta_{i}x_{i}+ \epsilon$$
