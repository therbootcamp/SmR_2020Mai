---
title: "Linear Models <i>applications</i>"
...
```{r, eval= FALSE, echo = TRUE}
ggplot(data = trial, mapping = aes(x = symptomatic, y = cd4_baseline)) + 
...
5. Using the `XXX$XXX` notation, print only the correlation estimate (`estimate`) in your `symptom_cd4_baseline_test` object. What does the result tell you?

```{r, eval = FALSE, echo = TRUE}
# Print parameter estimates in the symptom_cd4_baseline_test object
XXX$XXX
```

```{r}
# Print parameter estimates in the symptom_cd4_baseline_test object
symptom_cd4_baseline_test$estimate
```

6. Again using the `XXX$XXX` notation, print only the p value (`p.value`) in your `symptom_cd4_baseline_test` object. What does this value tell you?

```{r, eval = FALSE, echo = TRUE}
# Print the p value of the symptom_cd4_baseline_test object
XXX$XXX
```

```{r}
# Print the p value in the symptom_cd4_baseline_test object
...
```{r, eval= FALSE, echo = TRUE}
ggplot(data = trial, mapping = aes(x = molecule, y = days)) + 
  stat_summary(geom = "bar", fun.y = "mean", fill = "white", col = "black") +
  geom_jitter(width = .2, alpha = .1) +
  labs(title = "ACTG175 Trial",
       subtitle = "Baseline CD4 T cell count and symptoms",
...
3. Using `rsq()` calculate the r-squared value of your model. Was it high or low?

```{r, eval = FALSE, echo = TRUE}
rsq(XXX)
```

4. Using the code below, visualise the absolute residuals from the model. What does this mean?
