...
7. Sometimes it is usefull to extract the estimates from the model output (e.g., so we don't have to manually type them, which would be error prone). Extract the fixed effects from `subj_RI_mod` using the `fixef()` function.
...
3. Now fit the same model as in task C1, but add the `State` variables. Save the output as `RI_mod`. Note that we again need to use another optiomizer to avoid convergence problems (You don't need to worry about this now, you can use the code template below). 
