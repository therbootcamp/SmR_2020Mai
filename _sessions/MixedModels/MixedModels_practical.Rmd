...
2. Fit a mixed effects model predicting extraversion ("extra") with openness ("open") and aggreeableness ("agree") with random intercepts for classes nested in school and save it as `hier_mod`. *Hint*: The nested random effects structure is specified with the following syntax `(1|higher_level_variable/lower_level_variable)`.

```{r}
# Hierarchical mixed effects model
hier_mod <- lmer(extra ~ open + agree + # These are the fixed effects
...
```{r}
cr_mod <- glmer(remission ~ IL6 + CRP + CancerStage + LengthofStay + Experience +
...
The *Tomatometer_dat.csv* dataset contains tomatometer ratings from 
