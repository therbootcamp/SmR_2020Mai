...
# Load data
tom <- read_csv("1_Data/Tomatometer_dat.csv")
...
4. Using the following template, load the `Tomatometer_dat.csv` and `schools.csv` data into R and store it as a new object called `tom` and `schools`, respectively (Hint: Don't type the path directly! Use the "tab" completion!).
...
```{r}
# Load Tomatometer_dat.csv from the 1_Data folder
tom <- read_csv(file = "1_Data/Tomatometer_dat.csv")
...
|File | Rows | Columns |
|:----|:-----|:------|
|[Tomatometer_dat.csv](https://raw.githubusercontent.com/therbootcamp/SwR_2019Apr/master/1_Data/Tomatometer_dat.csv?token=AZQb9iWcc8yUCbFwBliP5UfVzAooM3okks5cmQ96wA%3D%3D) | `r nrow(tom)` | `r ncol(tom)` |
