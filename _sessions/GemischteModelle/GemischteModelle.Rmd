...
# [IID](https://en.wikipedia.org/wiki/Independent_and_identically_distributed_random_variables) Annahme

.pull-left45[
Bisherige Regressionsmodelle machen die Annahme, dass die Datenpunkte unabhängig und gleichverteilt sind (independent and identically distributed, <high>iid</high>).


* <high>Unabhängig</high>: Das Auftreten eines Wertes hat keinen Einfluss auf das Auftreten anderer Werte.
* <high>Gleichverteilt</high>: Alle Datenpunkte stammen aus derselben Wahrscheinlichkeitsverteilung.
...
- Leistung von Schülern in der PISA Studie(<high>Schüler > Klassen > Schulen > Ländern</high>).
- Multizentrische klinische Studie (<high>Patienten > Ärzte > Zentrum</high>)
- Vergleich unterschiedlicher Designs oder Geschmäcker eines Produkts mit <high>mehreren Einschätzungen pro Person</high>


**Wichtig**: *Verletzungen der IID-Annahme können zu irreführenden Resultaten führen, z.B. dem Überschätzen von Effekten.* 
...
mod_s1 <- lm(formula = Merkmahl ~ Dosis,
             data = DATEN_ZENTRUM1)

mod_s2 <- lm(formula = Merkmahl ~ Dosis,
             data = DATEN_ZENTRUM2)

mod_s3 <- lm(formula = Merkmahl ~ Dosis,
             data = DATEN_ZENTRUM3)
...
<high>Feste Effekte (fixed effects)</high>

* Die Globaleffekte, welche wir quantifizieren möchten.

<high>Zufällige Effekte (random effects)</high>

* xxx für die Abhängigkeiten in den Daten.
* Diejenigen Effekte, über welche wir generalisieren möchten. 


```{r eval = FALSE}
LMM <- lmer(formula = y ~ FE1 + FE2 + # Fixed
              (FE1|RE) + (FE2|RE), # zufällig
            data = DATEN)
```
...
<br>
Feste Effekte repräsentieren dieselben Gruppeneffekte wie bei den bisherigen Regressionsmodellen; zum Beispiel den Effekt eines Medikamentes auf ein relevantes Merkmahl.
...
In dieser Einführung arbeiten wir mit den folgenden Beispieldaten:
Wir sind Wissenschaftler und möchtet den Einfluss von Alkoholkonsum auf Filmrezensionen auf [Rotten Tomatoes](https://www.rottentomatoes.com/) untersuchen.

Dazu rekrutieren wir zwei Versuchspersonen, die zwei Filme beurteilen, und zwar einmal im nüchternen Zustand (`sober`) und einmal nachdem sie 4 Biere getrunken haben (`drunk`; mehr als 4 Biere würde als Binge Drinking zählen, wofür wir kein OK von der Ethikkommission erhalten würden).

* Wiederholte Messung (*within subjects design*)
* Dieselben Filme an beiden Zeitpunkten für beide VPs.
...
.pull-left45[
Mit zufälligen Effekten <high>kontrollieren wir für die "zufällige" Variation</high> in den Daten, welche von Quellen stammen, die Abhängigkeiten in den Daten verursachen. Häufig sind wir nicht an den konkreten Werten der zufälligen Effekte interessiert.


Zufällige Effekte:

* sind häufig der Teil, über welchen wir generalisieren möchten.
* sind immer kategoriale Variablen (<high>factors</high>)
* repräsentieren zufällige Ziehungen aus der Population.
...
* Geschätzte durchschnittliche Abweichung jedes Faktorlevels des zufälligen Effekts vom Intercept des festen Effekts.
    
    
<high>Random Slopes</high><br>
* Geschätzte Abweichung jedes Faktorlevels des zufälligen Effektes vom Slope des festen Effekts.

**Hinweis:**
Bei der Schätzung von random Intercepts wird nur <high>ein zusätzlicher Parameter</high> geschätzt: Die <high>Varianz</high> der Normalverteilung, aus welcher die random Intercepts stammen.
]
...
**Gekreuzte zufällige Effekte:** Alle Modellstrukuten mit multiplen zufälligen Effekten, welche nicht geschachtelt sind.
...
Es ist wichtig, die korrekte Struktur der zufälligen Effekte festzulegen. Wird dies nicht beachtet, kann dies zu einer $\alpha$-Fehler-Inflation oder einer niedrigen Power führen.
...
Jedoch kann die Spezifikation der maximalen Struktur der zufälligen Effekte zu konservativ sein, wenn diese nicht in den Daten vorhanden. Wir werden daher einen Kompromiss verwenden und ein <high>Rückwärtsauswahlverfahren</high> durchführen (siehe [Matuschek, Kliegl, Vasishth, Baayen, & Bates, 2017](https://www.sciencedirect.com/science/article/pii/S0749596X17300013)).
...
<br>
Wir wollen das <high>Tomatometer</high> Rating vorhersagen, mit <high>Status</high> als festen Effekt und mit random intercepts und random slopes über Rater (<high>ID</high>), sowie random intercepts und random slopes über Item (<high>Movie</high>). 
...
# Random intercepts und slopes über ID
                  (XXX) +
# Random intercepts und slopes über Movie
                  (XXX),
# Daten
                data = tomato)
...
<h1><a href="https://therbootcamp.github.io/SmR_2020Mai/_sessions/MixedModels/MixedModels_practical.html">Practical</a></h1>
