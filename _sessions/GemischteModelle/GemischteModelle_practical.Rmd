...
author: "<table style='table-layout:fixed;width:100%;border:0;padding:0;margin:0'><col width='10%'><col width='10%'>
  <tr style='border:none'>
    <td style='display:block;width:100%;text-align:left;vertical-align:bottom;padding:0;margin:0;border:none' nowrap>
      ...
      <a href='https://therbootcamp.github.io/SwR_2020Mai/'>
...
In diesem Practical wirst du die Anwendung gemischter Modelle üben. Dazu verwenden wir die `lme4`, `performance`, `parameters`, `stats`, und `pbkrtest` Pakete.
...
3. Lade mit `library()` die `tidyverse`, `lme4`, und `sjstats` Pakete.
...
**Tipp**: Verwende RStudios Autovervollständigung indem du *Tab* drückst.
...
5. Inspiziere die ersten Zeilen der Datensätze.
...
7. Verwende die `summary()` Funktion um deskriptive Statistiken der Variablen der beiden Datensätze zu erhalten.
...
8. Schaue dir die Datensätze mithilfe der `View()` Funktion an.
...
Im ersten Teil dieses Practicals arbeiten wir mit dem `tom` Datensatz aus den Folien und testen die Wirkung des `zustand`s einer Person (`nuechtern`  vs. `betrunken`) auf ihre `tomatometer`-Bewertung.
...
2. Im Moment wird "betrunken" als Referenzkategorie verwendet, sodass der intercept den Mittelwert des Zustands "betrunken" angibt und der slope die Differenz der Mittelwerte der Bewertungen im Zustand "betrunken" und "nuechtern" angibt. Um ein intuitiveres Modell mit "nuechtern" als Referenzkategorie zu erhalten, können wir wir die Variable  `zustand` in einen Faktor umwandeln und dort die Abfolge der Levels bestimmen. Verwende dafür den untenstehenden Code.
...
5. Rechne ein gemischtes Modell mit Random Intercepts über die Rater (Rater ids sind in der `id` variable gespeichert).
**Tipp**: Zufällige Effekte werden in Klammern in der Modell-`formula` in folgender Art spezifiziert `(`[Datenmatrix](https://de.wikipedia.org/wiki/Datenmatrix)`|Gruppierungsvariable)`.
**Hinweis**: Das `REML = FALSE` im untenstehenden Code sagt R, dass das Model mit Maximum Likelihood (ML), statt mit Restricted Maximum Likelihood (REML) gefitted werden soll. Dies ist für die späteren Modellvergleiche wichtig, soll uns aber im Detail nicht weiter beschäftigen (für mehr Informationen in einem eher technischen Ansatz siehe [hier](https://projecteuclid.org/euclid.cbms/1462106081) und [hier](http://www.stats.net.au/Maths_REML_manual.pdf), für einen weniger technischen Ansatz siehe [hier](https://en.wikipedia.org/wiki/Restricted_maximum_likelihood)).
...
8. Manchmal (zum Beispiel für Plots, oder damit wir sie nicht abschreiben müssen) ist es hilfreich, die Koeffizienten aus dem Modelloutput zu extrahieren. Extrahiere die Koeffizienten der festen Effekte aus `subj_RI_mod` mithilfe der `fixef()` Funktion.

```{r echo = TRUE, eval = FALSE}
# Extrahiere Koeffizienten der festen Effekte aus subj_RI_mod
fixef(XXX)
```
...
9. Extrahiere die Koeffizienten der zufälligen Effekte aus `subj_RI_mod` mithilfe der `ranef()` Funktion.

```{r echo = TRUE, eval = FALSE}
# Extrahiere Koeffizienten der zufälligen Effekte aus subj_RI_mod
ranef(XXX)
```
...
10. Baue dein gemischtes Modell aus Aufgabe B5 aus, indem du Random Slopes für `zustand` über `id`s hinzufügst.
**Tipp**: Füge die `zustand` Variable auf der Linken Seite der *Pipe* `|` in dem Teil der `formula`, in dem die zufälligen Effekte spezifiziert werden.
...
11. Vergleiche die Outputs der drei bisherigen Modelle miteinander. Verwende dazu die `summary()` Funktion. Haben sich die Koeffizienten verändert? Was hat sich verändert?
...
12. Jeder Film wurde in beiden Zuständen Bewertet. Wir haben also nicht nur Abhängigkeiten in den Daten der Rater, sondern auch der Filme. Um für diese Abhängigkeit zu kontrollieren, können wir unser Modell aus der Aufgabe B10 mit weiteren zufälligen Effekten ausbauen. Füge Random Intercepts über `film` und Random Slopes für `zustand` über `film` zum Modell hinzu.
**Hinweis**: Das zusätzliche Argument spezifiert einen anderen Optimisierungsalgorithmus, mithilfe dessen das Modell konvergiert. Die details sollen uns für den Moment nicht kümmern; wir freuen uns einfach, dass das Modell konvergiert.
...
max_mod <- lmer(formula = XXX ~ XXX + # Feste Effekte
               ...
               data = XXX,            # Daten
               REML = FALSE,
               control = lmerControl(optimizer = "bobyqa")) # Optimisierungsalgorithmus
```
...
13. Mit der `check_convergence()` Funktion des `performance` Paket können wir überprüfen, ob ein Modell konvergiert ist. Wenn die Funktion `TRUE` zurückgibt, ist alles in Ordnung. Teste, ob `max_mod` konvergiert ist.

```{r}
check_convergence(max_mod)
```

14. `max_mod` ist das *maximale, vom Design gerechtfertigten Modell*. Was heisst das? Weshalb macht es Sinn, dieses Modell zu spezifizieren?
...
15. Vergleiche mit `summary()` die Resultate der unterschiedlich Komplexen Modelle, die du bisher gerechnet hast.
```{r}
summary(max_mod)
summary(subj_RI_RS_mod)
summary(subj_RI_mod)
...
16. Was hat sich mit der zunehmend komplexen Struktur der zufälligen Effekte verändert?
...
1. Wir werden die `r2()` Funktion aus dem `performance` Paket verwenden, um die $R^2$ Werte zu berechnen. Schaue dir zunächst die Hilfedatei dieser Funktion an, um einen Überblick zu erhalten, mit welchen Modellarten sie angewendet werden kann.

```{r}
?r2
```

2. Berechne das Bestimmtheitsmass des maximalen Modells.

```{r}
r2(max_mod)
```

3. Was bedeutet der Output der vorherigen Aufgabe? Was ist das *marginal $R^2$* und was ist das *conditional $R^2$*?
...
4. Vergleiche die $R^2$ Werte von `max_mod` mit denen der weniger komplexen Modelle. Sind die Veränderungen gross? Welche $R^2$-Werte haben sich verändert?
...
Häufig ist es hilfreich, unser Modell zu visualisieren. Führe den untenstehenden Code aus, um aus `max_mod` Koeffizienten zu extrahieren und zu visualieren. Da Plotting kein Schwerpunkt dieses Kurses ist, gehen wir hier nicht aber nicht weiter ins Detail. Falls du Fragen zum Code hast, kannst du uns gerne rufen.
...
# besser darin, die Varianz zu erklären. "zustand" ist als 
```

#### Konfidenzintervall

Diese Methode ist berechnet keinen p-Wert, sondern die Konfidenzintervalle um die Parameter. Dann testen wir, ob 0 in Konfidenzintervallen enthalten ist. Falls nicht, betrachten wir die jeweiligen Zusammenhänge als signifikant.

6. Zur Berechnung von Konfidenzintervallen verwenden wir die `confint()` Funktion des `lme4` Pakets. Berechne damit die Konfidenzintervalle von `max_mod` und speichere den Output als `ci_mod`. **Achtung**: Das Berechnen der Konfidenzintervalle dauert eine Weile. 
...
8. Weitere Möglichkeiten p-Werte zu erhalten sind das Anschauen der t-Werte (t > 2 kann als Signifikanz interpretiert werden) oder einen [Wald test](https://en.wikipedia.org/wiki/Wald_test) mit der `p_value()` Funktion des `parameters` Pakets. Berechne mit `p_value()` die p-Werte von `max_mod`.
...
9. Statt einem Wald test wird auch die [Kenward-Roger Approximation](https://www.jstatsoft.org/article/view/v059i09) empfohlen. Um die p-Werte mit dieser Methode zu berechnen, können wir in der `p_value()` Funktion `method = "kenward"` setzen. Wiederhole die vorherige Aufgabe mit der Kenward-Roger Approximation. 

```{r}
# Berechne p-Werte für die festen Effekte
p_value(max_mod, method = "kenward")
```

10. Vergleiche die p-Werte der letzten beiden Aufgaben. Hat sich etwas geändert?

```{r}
# Es hat sich nichts grundlegend geändert. Der Grund dafür ist, dass alle
# Annahmen erfüllt waren, da die Daten so simuliert wurden. Bei verrauschten
# Daten kann es sein, dass hier Unterschiede auftauchen.
```
...
2. Benutze nun die `PBmodcomp()` Funktion um p-Werte für das Modell zu erhalten. Du wirst dazu ein weniger komplexes Modell spezifizieren müssen, gegen welches getestet werden soll. Benutze dazu `IO_mod`. Da dieses Vorgehen ziemlich lange dauern kann, benutze hier nur 100 Simulationen.
...
Die intra-Klassen Korrelation (ICC) ist "der Anteil der durch die in der Population vorhandenen Gruppenstruktur erklärten Varianz" (Hox, 2002, S. 15). Es wird berechnet, indem die Zwischengruppenvarianz durch die Summer der Innergruppen- und der Zwischengruppenvarianz (d.h., die totale Varianz) geteilt wird.

1. Berechne die ICC des Maximalmodells mit der `icc()` Funktion aus dem `sjstats` Paket.

```{r}
icc(max_mod)
```

2. Schaue dir das Ergebnis an. Hast du die Warnung bemerkt? Diese erschien, da die ICC nicht in derselben Weise interpretiert werden kann wie Random Slopes und Intercepts Modelle. Behebe die Warnung, indem du das Argument `adjusted = TRUE` in der `icc()` Funktion anfügst.

```{r}
icc(max_mod, adjusted = TRUE)
```

3. Schaue dir die `icc()` Hilfe-Seite an, um zu verstehen, was das Anfügen des `adjusted = TRUE` Arguments bewirkt hat.

4. Oft wir dei ICC baierend auf dem Reine Intercepts-Modell mit Random Intercepts berechnet. In Aufgabe C1 hast du dieses Modell bereits gerechnet und unter `IO_mod` abgespeichert. Berechne nun die ICC für dieses Modell. **Tipp**: Hier brauchst du das `adjusted` Argument nicht, da keine Random Slopes enthalten sind.

```{r}
icc(IO_mod)
```

5. Jetzt mache dasselbe für das Modell `RI_mod`, welches vom Modell `IO_mod`, welches du gerade benutzt hast, nur dadurch abweicht, dass es feste Effekte enthält.

```{r}
icc(RI_mod)
```

6. Vergleiche die Ergebnisse der beiden letzten Aufgaben. Hat sich etwas verändert? Überlege dir, was diese Änderungen zu bedeuten haben.
...
1. Wir werden nun mit dem `school` Datensatz arbeiten, welchen du bereits in Abschnitt A geladen hast. Erstelle unter Verwendung der `table()` Funktion eine Kreuztabelle von Schulen und Klassen.
...
5. Rechne nun dasselbe Modell nochmals, jedoch diesmal mit gekreuten random intercepts, wie in den vorherigen Abschnitten, anstelle der geschachtelten random intercepts. Speichere das Modell als `cross_mod`.
...
2. Führe nun das Vorgehen aus Abschnitt D durch um herauszufinden welches der beiden Modelle beibehalten werden sollte. **Hinweis**: Das Argument `df` in der `qchisq()` Funktion wird auf `df = 1` gesetzt, da diese beiden Modelle sich um nur einen Freiheitsgrad unterscheiden.

```{r}
# Alpha ist immer noch dasselbe
alpha <- .2 # Spezifiziere das Alpha-Niveau
st1 <- qchisq(1 - alpha, df = 1) # Ermittle den kritischen Wert über welchem die
...
2. Schaue dir die Daten an. Es handelt sich um einen Teil von simulierten Daten von [UCLA Institute for Digital Research and Education Search this website](https://stats.idre.ucla.edu/).
