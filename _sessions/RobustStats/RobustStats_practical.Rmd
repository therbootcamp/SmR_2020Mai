...
2. Wie du etablierte nicht-parametrische Statistiken implementierst und interpretirst.
...
1. In diesem Abschnitt geht us um die Evaluation der Annahmen der Regression ahand des `avocado_cali` Datensatzes, welcher Verkaufsdaten für Avocaods in Kalifornien über die letzten drei Jahre enthält. Das Ziel ist es zunächst `Verkaufsvolumen` über `Preis` vorherzusagen mit einer normalen Regression (`lm`) vorherzusagen.
...
3. In der Tat, `Preis` ist ein signifikanter Prädiktor mit einem sehr extremen t-Wert. Zudem weist `R-squared` einen hohen Wert von `62.8%` erklärte Varinz auf. Bedeutet dies nun, dass das Modell, welches einen linearen Zusammenhang annimmt, gut den Zusammenhang von `Preis` und `Verkaufsvolumen` beschreibt. Verwende den Code unten um die Daten zu plotten. 

. However, this does not imply that the relationship between average price and volume is described well by the regression line. Create a simple plot of the average price (x-axis) and volume (y-axis) using the template below. 
...
4. Irgendwie nicht so gut, oder? Übrprüfe dies indem du mit dem Code unten die Residuen gegen die gefitteten Werte plottest. 
...
5. Trotz des guten `R-squared` Werts liegen offensichtliche Verletzungen der Annahmen vor: Linearität, Normalität, und Homoskedaszität scheinen nicht zu halten. Ein Grund für diese Verletzungen sind fehlende Variablen. Ergänze das Modell einen weiteren Prädiktor, nämlich `Typ`, welcher die Art der Avocado kodiert und plotte erneut Residuen gegen gefittete Werte. Speichere das Modell als `cali_lm`.

```{r, echo = TRUE, eval = FALSE}
# Regression Verkausvolumen auf Preis & Typ
cali_lm <- lm(formula = XX ~ XX + XX, data = XX)
ggplot(mapping = aes(fitted(XX), resid(XX)))
```
...
8. Die Punkte streuen nun etwas gleichmässiger um die Null-Gerade. Ganz ideal ist das Bild aber immer noch nicht, denn offensichtlich ist der Varianz der Residuen im unteren Bereich höher als im oberen Bereich. Ergo ist Homoskedaszitität verletzt. Rechne nun eine Regression mit `Verkaufsvolumen_index` (ohne `log()`) und überprüfe, ob das vielleicht die verbleibenden Problme löst. 
...
9. Vielleicht ist jetzt die Varianz etwas ausgegeglichener, ideal sieht es aber immer noch nicht aus. Ganz perfekt ist das aber immer noch nicht. Du kannst dies mit dem Code unten, welcher die Standardabweichung separat für die beiden Punktwolken bestimmt, überprüfen. 
...
10. Versuche schlussendlich die Poisson Verteilung mit `Verkaufszahlen` (einer Häufigkeitsvariable) als Kriterium.
...
11. Man könnte man dies hätte die Angelegenheit weiter verschlechtert. Tatsächlich ist dies aber nicht so. Die Poisson-Regression hat andere Annahmen, als die normale Regression. Die normale Regression nimmt an, dass die Varianz konstant um die Regressionsgerade verteilt ist, die Poisson-Regression dagegen, dass die Varianz mit zunehmenden vorhergesagtem Wert ansteigt. Dies entspricht in diesem exakt dem beobachteten Plot, womit wir belegt hätten, dass man Häufigkeitsverteilungen vielleicht doch am besten mit der davor vorgesehenen Verteilung modelliert. 
...
2. Vergleiche nun mit einem `t.test()` das `Verkaufsvolumen` und den `Verkaufsvolumen_index` zwischen den beien Jahren. Setze dabei `paired = TRUE` um zu berücksichtigen, dass es sich hier um abhängige Daten handelt. Siehe Template.
...
5. Auch hier zwei signifikante Ergebnisse. Nun kannst du langsam tatsächlich davon ausgehen, dass der Verkauf im Jahre 2017 gegenüber 2016 leicht zurück gegenagen ist.

### D - Robust regression

1. Begib dich nocheinmal zurück zur Vorhersage des `Verkaufsvolumen` durch `Preis` und `Typ`. Ein weiterer Weg gegenüber denen bisher eingeschlagenen um mit potentiellen Verletzungen der Regression umzugehen sind robuste Regressionen. Rechne zunächst noch einmal das ursprüngliche Modell mit einer normalen Regression. 
...
3. Printe das `B` und evaluiere das Ergebnis. Die Spalte `original` enthält die Gewichte für den gesamenten Datensatz. Die Spalte `bias` den Unterschied zwischen dem Mittel der Gewichte über die Bootstrap Samples und der Spalte `original`. Diese sollte möglichst klein sein. Die Spalte `std. error` enthält das Ergebnis von Interesse: den über Bootstrap geschätzten Standardfehler der Gewichte. 
...
4. Das Ergebnis des Bootstrap kannst du nun einfach über eine z-Verteilung testen a la `z = b / se`, d.h., das Gewicht geteilt durch den Standardfehler folgt der z-Verteilung. Der Code unten sieht die relevanten Werte aus dem Objekt und berechnet die z-Werte für Intercept und die zwei Gewichte. 
...
1. In dieser letzten Sektion, hast du die Möglichkeit nochmal gemischte Modell zu wiederholen. Unten steht ein template um ein gemischtes Modell für den gesamten Avocado Datensatz zu rechnen in dem du berücksichtigts, dass die Daten aus verschiedenen Regionen stammmen. Have fun! 

```{r, echo = TRUE, eval = FALSE}
# model
avocado_lmer <- lmer(formula = Verkaufsvolumen ~ XX + (1 + XX | Region), 
                     data = avocado)

# residual plot
plot(x = fitted(avocado_lmer), y = residuals(avocado_lmer))
```

2. Natürlich kannst du auch wiederum den Residualplot überprüfen. Diesmal separat für jede Region. 

```{r, echo = TRUE}
# Residualplot
ggplot(tibble(fitted = fitted(avocado_lmer), 
              resid = resid(avocado_lmer), 
              Region = avocado$Region),
       mapping = aes(fitted, resid)) + 
  geom_point() + 
  facet_wrap(~Region)
...
Die Datensätze entstammt der [Hass](https://en.wikipedia.org/wiki/Hass_avocado) Avocado Board Webseite und enthalten wochenweise Avocado Verkaufsvolumina von April 2015 bis May 2018 für verschiedene Regionen der USA. Der kleinere Datensatz enthält nur die Daten für Kalifornien.   
