...
subtitle: "Testing regression assumptions, non-parametric tests, and bootstrapped analyses"
...
var_res1 <- var(x = residuals(XX))
var_res2 <- var(x = residuals(YY))
...
var_res1 <- var(x = residuals(m3))
var_res2 <- var(x = residuals(m4))
var_res1
var_res2
```
...
boot_lm <- function(data, indices){
  ...
  m <- lm(vvolume_index ~ price_per_avocado + type, 
          data = data)
...
# residual plot
plot(x = predict(mm), y = residuals(m,))
```
