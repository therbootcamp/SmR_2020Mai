...
In this hour, we will try to cover the basics of statistical inference
...
knitr::include_graphics("https://raw.githubusercontent.com/therbootcamp/SwR_2019Apr/master/src/img/variability_stock_drug_beer.png?token=AIFo1GDIBmjgX1ca13vVfnh0UqoEa-93ks5cqiplwA%3D%3D")
```
...
```{r, echo = FALSE, fig.width = 6, fig.height = 3, dpi= 200, out.width = "100%"}
...
happiness <- .2 * exercise + rnorm(50, mean = 0, sd = 1) - 2

dat <- tibble(id, income, exercise, happiness, group = rep("Baselers", 50))
...
low <- ggplot(dat, aes(income, happiness)) +
  geom_point() +
...
high <- ggplot(dat, aes(exercise, happiness)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(y = "Happiness",
       x = "Minutes of Excercise per day",
...
ggarrange(low, high, ncol = 2, nrow = 1)
...
ggplot(beer_data, aes(x = id, y = beer)) +
  ...
  ylim(c(20, 40)) +
  geom_hline(yintercept = mean(beer), col = baselrbootcamp_cols("green"), size = 1.5) +
...
```{r, echo = FALSE, out.width = "100%"}
knitr::include_graphics("https://raw.githubusercontent.com/therbootcamp/SwR_2019Apr/master/src/img/10_beers_teststatistic.png?token=AIFo1A8r2pk1_0TJe-dFnaeuJUOw88X1ks5cqiBmwA%3D%3D")
```
